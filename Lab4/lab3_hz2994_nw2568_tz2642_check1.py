...
# Button pins
button_inc = Pin(2, Pin.IN, Pin.PULL_UP)  # To increase time
# button_dec = Pin(13, Pin.IN, Pin.PULL_UP)  # To decrease time
button_mode = Pin(0, Pin.IN, Pin.PULL_UP)  # To select mode
...
def display_time(x,y):
    ...
    oled.text(formatted_date, x%128, y%32)
    oled.text(formatted_time, x%128, (y+12)%32)
    oled.text("Mode: " + modes[mode_idx], x%128, (y+24)%32)
